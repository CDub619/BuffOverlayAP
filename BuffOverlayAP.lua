...
hooksecurefunc("CompactUnitFrame_UpdateBuffs", function(self)
	...
	if not overlay then
		...
		overlay = CreateFrame("Button", "$parentBuffOverlayAP", self, "CompactAuraTemplate")
		overlay:ClearAllPoints()
		overlay:SetPoint("BOTTOMRIGHT", self, "BOTTOMRIGHT", -15, 95)
	...
	if index then
		...
		--overlay.icon:SetVertexColor(0.78,	0.61,	0.43);
		if ( count > 1 ) then
